...
router.post('/',
    contactLimiter,
    [
        body('name').trim().isLength({ min: 3, max: 150 }).withMessage('Nome inválido'),
        body('email').isEmail().normalizeEmail().withMessage('E-mail inválido'),
        body('subject').trim().isLength({ min: 5, max: 200 }).withMessage('Assunto inválido'),
        body('message').trim().isLength({ min: 20, max: 5000 }).withMessage('Mensagem inválida'),
    ],
